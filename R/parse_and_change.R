parse_and_change <- function(df, var, date_col, date_format, what_change){
  df%>%
    ...
    what_lag=case_when(mean(month_diff,na.rm=TRUE)==1 & what_change=="annual" ~ 12,
                       mean(month_diff,na.rm=TRUE)==12 & what_change=="annual" ~ 1,
                       mean(month_diff,na.rm=TRUE)==3 & what_change=="annual" ~ 4,
